...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        if self.training and targets is None:
            raise ValueError("In training mode, targets not empty ")
        ...
        if torch.jit.is_scripting():
            if not self._has_warned:
                warnings.warn("RCNN always return a (losses Detections)")
...
class TwoMLPHead(nn.Module):
    ...
    def __init__(self, in_channels, representation_size):
        ...
        self.fc6 = nn.Linear(in_features=in_channels, out_features=representation_size)
        self.fc7 = nn.Linear(in_features=representation_size, out_features=representation_size)
...
class FastRCNNPredictor(nn.Module):
    ...
    def __init__(self, in_channels, num_classes):
        ...
        self.cls_score = nn.Linear(in_features=in_channels, out_features=num_classes)
        self.bbox_pred = nn.Linear(in_features=in_channels, out_features=num_classes * 4)
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
                 ...
                 # rpn中在nms处理前保留的proposal数(根据score) 每一层特征图2k
                 rpn_pre_nms_top_n_train=2000, rpn_pre_nms_top_n_test=1000,
                 # rpn中在nms处理后保留的proposal数,所有特征层总共2k
        ...
        roi_heads = RoIHeads(
            ...
            box_batch_size_per_image, box_positive_fraction,  # 512 0.5
            bbox_reg_weights,
            box_score_thresh, box_nms_thresh, box_detections_per_img  # 0.05 0.5 100
        )
        if image_mean is None:
            image_mean = [0.0485, 0.456, 0.406]
