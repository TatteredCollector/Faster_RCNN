...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    """
    :param class_logits: 预测类别概率信息，shape=[num_anchors, num_classes]
    :param box_regression: 预测边目标界框回归信息
    :param labels: 真实类别信息
    :param regression_targets: 真实目标边界框信息
    ...
    # shape=[num_proposal, num_classes,4]
    box_regression = box_regression.reshape(N, -1, 4)

    # 计算边界框损失，这里认为负样本也参与了损失计算loss=0
...
class RoIHeads(torch.nn.Module):
    __annotations__ = {
        ...
        'fb_bg_sampler': det_utils.BalancedPositiveNegativeSampler,
    }
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        # 遍历每一张图像的proposals,gt_boxes,gt_labels信息
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            if gt_boxes_in_image.numel() == 0:  # 该图没有gt,为背景
                # background image
                ...
                # 将gt索引-1的类别设置为0，为背景，负样本
                bg_inds = matched_idxs_in_image == self.proposal_matcher.BELOW_LOW_THRESHOLD  # -1
                labels_in_image[bg_inds] = 0

                # 将gt索引-2的类别设置为-1，即废弃样本
                ignor_inds = matched_idxs_in_image == self.proposal_matcher.BETWEEN_THRESHOLD  # -2
                labels_in_image[ignor_inds] = -1
    ...
    def subsample(self, labels):
        ...
        for img_idx, (pos_inds_img, neg_inds_img) in enumerate(zip(sampled_pos_inds, sampled_neg_inds)):
            # 记录所有采集样本索引 where返回的是一个List[Tensor]所以要取出来
    ...
    def select_training_samples(self,
                                ...
                                targets,  # type:Optional[List[str,Tensor]]
                                ):
        ...
        # # 如果不加这句，jit.script会不通过
        assert targets is not None

        dtype = proposals[0].dtype
        device = proposals[0].device

        # 获取标注好的boxes 以及labels信息
        gt_boxes = [t['boxes'].to(dtype) for t in targets]
        gt_labels = [t['labels'] for t in targets]

        # 将gt_box拼接到proposals后面 增加正样本数
        proposals = self.add_gt_proposals(proposals, gt_boxes)

        # 为每一proposal匹配对应的gt_box，并划分正负样本
        ...
        # 根据gt 和proposals 计算边框回归参数，正负样本都会计算，后续会筛选
    ...
    def postprocess_detection(self,
        ...
        # type:(...)->Tuple[List[Tensor],List[Tensor],List[Tuple]]
        """
         对网络的预测数据进行后处理，包括
        ...
        # 根据proposal以及预测回归参数解码计算出最终的bbox坐标
        pred_boxes = self.box_coder.decode(box_regression, proposals)
        # 对预测类别结果 ，每一个proposal进行softmax
        pred_score = F.softmax(class_logits, -1)

        # 根据每张图像的预测bbox数量分割结果：
        pred_boxes_list = pred_boxes.split(boxes_per_image, 0)
        pred_score_list = pred_score.split(boxes_per_image, 0)

        all_boxes = []
        all_scores = []
        all_labels = []
        # 遍历
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_score_list, image_shapes):
            # 裁剪预测的bboxes,限制越界
            boxes = box_ops.clip_boxes_to_image(boxes, image_shape)

            # 类别标签拓展 (0,1,2,...,num_class-1)
            labels = torch.arange(num_classes, device=device)
            # (proposals_num,num_class)
    ...
    def forward(self,
        ...
        :param proposals:  (List[Tensor[N, 4]])
        :param image_shapes:经过预处理后的大小 (List[Tuple[H, W]])
        ...
        # 接着分别预测目标类别和边界框回归参数
        # class_logits, box_regression:[num_proposals,num_class+1],[num_proposals,(num_class+1)*4]
        ...
        if self.training:
            assert labels is not None and regression_targets is None
